...
public:
    ...
    //TODO : check if this is correct, AI generated
    ExtractedSequenceInfo  extract_sequence(MetaSolution& metaSolution, const DataInstance& instance, int scenario_id, int match_count, const std::vector<std::vector<int>>* seq_ptr, const std::vector<int>* times_ptr) const override{
        if(metaSolution.scored_by){throw std::runtime_error("Extracting sequence but metasol is already scored.");} //if already scored, why don't we just get the stored result?

        ExtractedSequenceInfo output_info; // Our new return struct
        output_info.sequence = Sequence(); // Initialize
        output_info.last_node = -1; // Default: no node to start from (if match_count was 0)
        bool set_output = false;
        ...
        if (auto* groupMeta = dynamic_cast<GroupMetaSolution*>(&metaSolution)) {
            ...
            std::vector<int> current_group_flat_sequence; // To store the sequence of tasks *within* processed groups
            std::vector<int> current_group_end_times; // To store the end times for each processed group
            if (match_count > 0 && seq_ptr != nullptr) {
                // Use cached sequence up to match_count
                sequence = (*seq_ptr)[scenario_id];
            }
            sequence.resize(instance.N);
            int c = sequence.size(); // counter starts from where cached sequence ends
            const auto& releaseDates = instance.releaseDates[scenario_id];
            const auto& durations = instance.durations;
            const auto& prec = instance.precedenceConstraints;
            std::priority_queue<int, std::vector<int>, std::greater<int>> free_nodes; // sorted queue of available (both release and precednece wise) nodes (default comparison by index) sorted by spt (through index of group); 
            int time  =  (match_count > 0) ? (*times_ptr)[scenario_id] : 0; //tracks time to see if tasks are ready (if cached, use cached time)
            std::vector<int> incoming_edges_nb; 
            std::vector<std::vector<int>> outgoing_edges;

            // Process remaining groups after match_count
            auto groups = groupMeta->get_task_groups_modifiable();
            for (size_t g = match_count; g < groups.size(); g++) {
                auto& group = groups[g];
                //creating the second set here due to scope issue, but some overhead is expected.
                auto releaseDateComparator = [&](int index1, int index2) {
                    return (releaseDates[group[index1]] < releaseDates[group[index2]]) || ((releaseDates[group[index1]] == releaseDates[group[index2]]) && (group[index1]<group[index2])); //lex if equal (could be unnecessary, but I'm afraid of undefined behavior if weak ordering)
